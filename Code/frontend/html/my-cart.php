...
async function applyVoucher() {
  ...
  try {
    ...
    if (result.status === "success") {
      // voucher info returned by server
      const v = result.voucher || {};

      // ensure the code is present (server may or may not return it)
      v.code = v.code || code;

      // compute current cart grand total robustly
      ...
      if (v.type === "discount") {
        ...
        // adjust this if you have a real shipping calc
        discountAmount = 50;
      }
      ...
      // Store voucher with helpful fields so checkout can use it
      const stored = {
        code: v.code,
        type: v.type,
